 ...
 *</ul>
 */
public class ChunkAnalysisAndPOSSequenceValidator implements SequenceValidator<String> {
	
	private String label;
	
	public ChunkAnalysisAndPOSSequenceValidator(String label) {
		this.label = label;
	}
	...
	public boolean validSequence(int index, String[] words, String[] posChunkTags, String out) {
		...
		switch(label) {
		case "BIEO":
			res = isBIEO(chunkTags, index, chunk);
			break;
		case "BIO":
			res = isBIO(chunkTags, index, chunk);
			break;
		default:
